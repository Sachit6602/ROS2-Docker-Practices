...
class bot_pathplanner():
    ...
    #Reverses (x,y) to (y,x), which needed for image indexing(row, col)
    @staticmethod
    ...
    def draw_path_on_maze(self,maze,shortest_path_pts,method):
        #convert grayscale maze to RGB color
        maze_bgr = cv2.cvtColor(maze, cv2.COLOR_GRAY2BGR)
        #initializes a blank image(same shape as maze) to draw only the path
        self.choosen_route = np.zeros_like(maze_bgr)

        rang = list(range(0,254,25))
        
        depth = maze.shape[0]
        #for each line segment in the path, compute how far it is vertically in the maze
        for i in range(len(shortest_path_pts)-1):
            ...
            #draw lines btw points in both maze and blank path image
        ...
        #save the image to corresponding algorithm's class
        if method == "dijisktra":
    ...
    def find_path_nd_display(self,graph,start,end,maze,method = "DFS"):
        ...
        #calls A*
            ...
            #convert coordinates and store the path
